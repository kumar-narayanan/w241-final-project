...
ggplot(d_treat_ctrl , aes(x = treatment, y =counts, 
  ...
  labs(title='Treatment Vs Control counts')
```
...
for (city in cities) {
  mean_inc <- c(mean_inc, d[geography==city, as.character(mean(income))])
...
p1 <- ggplot(df, aes(x=mean_inc, y = mean_int, fill=location)) + 
  geom_bar(stat="identity") +
  labs(x = "Mean income", y = "Mean interest level in AD", 
       title = "Mean interest level in AD Vs Mean imcome by location")
